...
/// pixel.
fn validate_masks_for_bpp(masks: &[(u32, ColorMaskChannel)], bpp: u16) -> BmpResult<()> {
    ...
    if bpp > 32 {
        return Err(BmpError::InvalidBitCount(bpp));
    }

    let pixel_mask = if bpp == 32 { u32::MAX } else { (1u32 << bpp) - 1 };
