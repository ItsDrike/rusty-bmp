//! Top toolbar for file loading and save controls.
...
impl BmpViewerApp {
    ...
    pub(in crate::gui) fn show_toolbar(&mut self, ctx: &egui::Context) {
        egui::TopBottomPanel::top("top").show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                egui::ComboBox::from_id_salt("save_header_version")
                    .selected_text(self.save.save_header_version.to_string())
                    .show_ui(ui, |ui| {
                        for &ver in SaveHeaderVersion::ALL {
                            ui.selectable_value(&mut self.save.save_header_version, ver, ver.to_string());
                        }
                    });
                self.save.set_header_version(self.save.save_header_version);
                ui.label("Format:");
                egui::ComboBox::from_id_salt("save_format")
                    .selected_text(self.save.save_format.to_string())
                    .show_ui(ui, |ui| {
                        for &fmt in self.save.save_header_version.compatible_formats() {
                            ui.selectable_value(&mut self.save.save_format, fmt, fmt.to_string());
                        }
                    });
